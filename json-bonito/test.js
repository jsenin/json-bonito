...
test('detects tuples', () => {
  eq(looksLikePython("{'t': (1, 2)}"), true);
});

test('detects Python True keyword', () => {
  eq(looksLikePython("{'a': True}"), true);
});

test('detects Python False keyword', () => {
  eq(looksLikePython("{'a': False}"), true);
});

test('detects Python None keyword', () => {
  eq(looksLikePython("{'a': None}"), true);
});

test('rejects valid JSON', () => {
  eq(looksLikePython('{"a": 1}'), false);
});

test('Python keywords inside strings are not detected', () => {
  eq(looksLikePython("{'x': 'True'}"), false);
});

test('detects mixed Python and JSON', () => {
  eq(looksLikePython("{'a': True, 'b': 'hello'}"), true);
});
